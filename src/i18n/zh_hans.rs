...
// ── 处置选项（静态） ──────────────────────────────────────────────────────────
pub const OPT_RESTORECON_DESC:     &str = "首先尝试此方法。恢复默认文件上下文（修复标签丢失问题）。";
pub const OPT_CUSTOM_POLICY_LABEL: &str = "生成并应用自定义策略模块（audit2allow）";
pub const OPT_CUSTOM_POLICY_DESC:  &str = "使用 audit2allow 自动生成策略。如果知道路径，请先按 P 键尝试更好的修复方案。";
pub const OPT_PERMISSIVE_DESC:     &str = "⚠ 禁用此域的所有拒绝。严重降低安全性，仅用于调查。";
...
pub const ANALYSIS_PATH_UNKNOWN_HINT: &str = " ※ 路径未知。按 P 键指定目录路径以查看最佳修复方案。";
pub const PATH_INPUT_TITLE:  &str = " 输入目录路径";
pub const PATH_INPUT_PROMPT: &str = " 请输入绝对路径（例: /var/log/myapp）";
pub const PATH_INPUT_HINT:   &str = " Enter: 确认  Esc: 取消";
pub const OPT_PATH_INPUT_LABEL: &str = "输入绝对路径以启用 restorecon/fcontext";
pub const OPT_PATH_INPUT_DESC:  &str = "路径未知，无法显示修复选项 A/B。输入绝对路径后，将显示标签修复步骤（restorecon / semanage fcontext）。";
...
pub fn analysis_dir_label_check(dir: &str) -> String {
    format!(" 使用 ls -dZ {} 确认目录标签。如标签有误，请先尝试 restorecon。", dir)
}
