...
pub const HINT_MODULE_LIST:  &str = "↑↓/jk:Перемещение  d:Удалить  a:Все/Польз.  Esc:Назад";
pub const MODULE_LIST_AUTH_DESC: &str = "Для получения списка модулей требуются права администратора. После аутентификации загрузится автоматически.";
pub const HINT_MODULE_DETAIL:    &str = "↑↓/jk:Прокрутка  Esc:Назад";
pub const MODULE_DETAIL_AUTH_DESC:  &str = "Для получения сведений о модуле требуются права администратора. После аутентификации загрузится автоматически.";
pub const MODULE_DETAIL_TITLE:      &str = " Детали модуля ";
pub const MODULE_DETAIL_CIL_TITLE:  &str = " CIL правила ";
...
pub fn module_list_title(custom: usize, total: usize, show_all: bool) -> String {
    if show_all {
        format!(" Модули политики  {} шт. ", total)
    } else {
        format!(" Польз. модули  {} / всего {} ", custom, total)
    }
}
...
pub fn log_overlay_title(total: usize) -> String {
    format!(" Журнал  {} записей  ↑↓:Прокрутка  Esc:Закрыть ", total)
}
