...
- **メモリ保護:** sudo パスワードは [`zeroize`](https://docs.rs/zeroize) クレートで管理し、`drop` 時にメモリがゼロ化されます。
- **ブルートフォース保護:** 認証失敗 3 回で 60 秒のロックアウトが発生します。
- **セキュアな実行:** パスワードは stdin 経由で `sudo -S -k` に渡し、プロセス引数には含まれません。
- **ロケール隔離:** 外部コマンド実行時に `LC_ALL=C` を強制し、システム言語設定に依存しない安定したパースを実現しています。
- **安全な一時ファイル:** `audit2allow` の中間生成物には `tempfile` を使用し、予測不可能なパスで競合や攻撃を防ぎます。
- **XDG 準拠:** ログやデータ保存先は XDG Base Directory 規約（`dirs` クレート）に従います。
