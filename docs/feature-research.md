...
#### D. `semanage fcontext` + `restorecon` 2ステップ修復
...
#### E. `semodule -X 300` の適用

**概要：** カスタムポリシーのインストール時に優先度 300 を指定する。

**現状：** `semodule -i NAME.pp`
...
#### F. first_seen タイムスタンプの表示

**概要：** ausearch 出力の最初のレコードのタイムスタンプを `first_seen` として保存し、詳細画面に「初回発生 / 最終発生」を両表示。

**効果：** 断続的な問題の時間的スコープを把握できる
...
**概要：** Boolean remedy 時に `sepolicy booleans -b BOOLNAME` を呼び出し、その boolean を有効にすると何が許可されるかの説明文を詳細画面に表示。
...
**概要：** `semodule -l` の出力一覧を表示する新規画面を追加。`semodule -r NAME` で削除可能。

**効果：** seadmin で適用した audit2allow モジュールのクリーンアップが可能になり、管理のクローズドループが完成する
...
#### J. syscall 名・errno 名の変換表示

**概要：** audit ログの `syscall=NNN` を `read` / `open` / `bind` 等のシンボル名に変換、`exit=-13` を `EACCES` 等に変換して詳細画面に表示。

**難易度：** 小

---

#### K. 全ファイルシステム再ラベル（`.autorelabel`）
...
| # | 機能 | 難易度 | 効果 | 推奨順位 |
|---|---|---|---|---|
| A | audit2why 統合による原因診断精度向上 | 中 | 大 | 1 |
| B | 動的 Boolean 検出 | 小 | 大 | 2 |
| E | semodule -X 300 適用 | 極小 | 中 | 3 |
| D | semanage fcontext + restorecon 2ステップ修復 | 小 | 中 | 4 |
| C | severity レベル表示（緑/黄/赤） | 小 | 中 | 5 |
| F | first_seen タイムスタンプ表示 | 小 | 中 | 6 |
| G | Boolean 説明文表示 | 小 | 中 | 7 |
| H | ポリシーモジュール管理画面 | 大 | 中 | 8 |
| I | sepolicy network によるポート情報補完 | 中 | 小 | 9 |
| J | syscall / errno 名変換表示 | 小 | 小 | 10 |
| K | 全ファイルシステム再ラベル | 小 | 小 | 11 |

---

## 5. 参考：現在の seadmin 機能サマリ

### 画面構成

| 画面 | 概要 |
|---|---|
| AVC 一覧 | ausearch 結果を集約表示、`/` でフィルタ |
| AVC 詳細 | 原因分析・修復オプション選択・生ログ表示 |
